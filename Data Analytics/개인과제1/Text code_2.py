...
for i in tqdm(range(0,len(rawdata))):
    ...
    if sp==[1,1]:
        ...
        sp=[0,0]
def tokenizer(raw, pos=["Noun","Alpha","Number"], stopword=["based","method","multi","multiple","processing","providing","service","surpporting","systems","via","using","use","user","apparatus","device","system","supporting"]):
    return [
...
df.to_csv("Text_code_result_2.csv",encoding="utf-8-sig",index = features,header = features)