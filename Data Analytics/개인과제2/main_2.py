...
tfidfv = TfidfVectorizer(min_df=10).fit(reveiw_Total_data)
X = tfidfv.transform(reveiw_Total_data).toarray()
...
review_X_data_df.to_csv("review_x_data_2.csv",encoding="utf-8-sig",header = features)
review_O_data_df=review_O_data_df.head(len(review_O_data))
review_O_data_df["태그"] = review_O_data_tag
features.append("태그")
review_O_data_df.to_csv("review_o_data_2.csv",encoding="utf-8-sig",header = features)
review_O_data_df=pd.read_csv('./review_o_data_2.csv')
review_X_data_df=pd.read_csv('./review_x_data_2.csv')
...
stoplist=["제","걸","번","그것","요","거","온","함","분","하다","임","것","다시다","전","치","뿌","링클","식","쿡","바"]
#결과를 계속 보고 stoplist를 채워준다.
texts = [[word for word in document.lower().split() if word not in stoplist] for document in documents]
dictionary = corpora.Dictionary(texts)
corpus = [dictionary.doc2bow(text) for text in texts]
lda = gensim.models.ldamodel.LdaModel(corpus=corpus, id2word=dictionary,
                                      num_topics=8, update_every=1, chunksize=100, passes=30)
result=lda.print_topics(num_words=10)
for i in result:
    print(i)
